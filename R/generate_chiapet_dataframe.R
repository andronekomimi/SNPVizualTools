...
for (current_chr in chr_list) {
  ...
  create_df_from_sfd <- function(file_name) {
    ...
    data2 <- strsplit(gsub("\\.\\.", "-", df2[,4]), '[:,-]')
    df2 =  matrix(unlist(data2), ncol = 7, byrow = TRUE)
    saveRDS(df2, file=paste0("~/Workspace/SNPVIZU/SNPVizualTools/data/",current_chr,"_",file_name,".Rda"))
  }
  
  create_df_from_sgp_inter <- function(file_name) {
    my.file <- paste0(sgp_files_path[[file_name]], current_chr,"_inter")
    df3 <- read.table(my.file, header=FALSE, stringsAsFactors=FALSE)
    data3 <- strsplit(gsub("\\.\\.", "-", df3[,4]), '[:,-]')
    df3 =  matrix(unlist(data3), ncol = 7, byrow = TRUE)
    saveRDS(df3, file=paste0("~/Workspace/SNPVIZU/SNPVizualTools/data/",current_chr,"_",file_name,"_inter.Rda"))
    
  }
  
  mclapply(names(sgp_files_path), create_df_from_sgp, mc.cores = 2)
  mclapply(names(sdf_file_path), create_df_from_sfd, mc.cores = 2)
  mclapply(names(sgp_files_path), create_df_from_sgp_inter, mc.cores = 1)
} 
